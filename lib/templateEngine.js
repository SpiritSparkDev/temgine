...
function processNavigationPlaceholders(templateCode, pages, navigationTemplates = {}) {
  ...
  return templateCode.replace(navigationRegex, (match, navName) => {
    ...
    if (navTemplate) {
      try {
        // Debug: indicate navigation template is being rendered
        // eslint-disable-next-line no-console
        console.debug(`processNavigationPlaceholders: rendering navigation '${navName}' with ${Array.isArray(pages) ? pages.length : 0} pages`)

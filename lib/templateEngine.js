...
const applyDynamicBlockSlots = (templateCode, blockHtmlByTemplate = {}, blockSlotConfig = {}) => {
  if (!templateCode || typeof templateCode !== 'string') {
    return { templateCode, slotData: {} }
  }
  ...
  const nextCode = String(templateCode).replace(dynamicSlotRegex, (match, rawSlotName) => {
    ...
    const selectedHtml = selectedTemplate ? (blockHtmlByTemplate[selectedTemplate] || '') : ''
...
export function renderPage(page, blockTemplates = {}, pageTemplateCode = null, allPages = [], navigationTemplates = {}, templateTemplates = {}, snippets = {}, options = {}) {
  ...
  renderedTopBlocks.forEach(({ templateName, html }) => {
  ...
  if (pageTemplateCode) {
    ...
    const dynamicBlockSlotResult = applyDynamicBlockSlots(pageTemplateCode, blockHtmlByTemplate, dynamicBlockSlotConfig)
    ...
    const blocksWrapped = (options && (options.pageBlocksWrapperBefore || options.pageBlocksWrapperAfter)) ? ((options.pageBlocksWrapperBefore || '') + blocksHtml + (options.pageBlocksWrapperAfter || '')) : blocksHtml
    const pageData = {
      ...(page.data || {}),

...
export function renderPage(page, blockTemplates = {}, pageTemplateCode = null, allPages = [], navigationTemplates = {}, templateTemplates = {}, snippets = {}) {
  ...
  const renderBlockRecursive = (block) => {
    ...
    if (templateName && blockTemplates[templateName]) {
      ...
      const childrenHtml = (block.children || []).map(renderBlockRecursive).join('')
      ...
      // append the children HTML so nested blocks are visible by default.
      try {
        if (childrenHtml && String(childrenHtml).trim()) {
          ...
          if (!hasInnerPlaceholder) {
            ...
            // If the rendered block starts with a tag like <article>...</article>
            // insert the childrenHtml before the corresponding closing tag so
            // the children become true DOM children of the parent element.
            try {
              ...
              if (openingMatch) {
                ...
                if (idx !== -1) {
                  ...
                  return before + `<div class="block-children">${childrenHtml}</div>` + after
                }
              }
            } catch (e) {
  ...
  const blockHtmls = (page.blocks || []).map(renderBlockRecursive).filter(html => html)

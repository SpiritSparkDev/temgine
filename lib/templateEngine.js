...
export function renderTemplate(templateCode, data) {
  try {
    ...
    let cleaned = withEachExpanded.replace(typeAnnotationRe, '$1$2$4');

    // If a value already contains HTML, force unescaped output for that key.
    // This prevents visible "<p>..." text when templates still use {{key}}.
    ...
    return Mustache.render(cleaned, data);
  } catch (error) {

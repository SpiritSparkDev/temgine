...
export function extractTemplateVariables(templateCode) {
  ...
  // appearing both outside AND inside an each block is not incorrectly excluded.
...
export function extractFieldGroups(templateCode) {
  ...
  // Strip each blocks so their inner fields aren't also rendered as top-level groups
  // (they're rendered separately via extractRepeaterBlocks / "Eintrag hinzufügen").

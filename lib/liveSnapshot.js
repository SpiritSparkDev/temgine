...
import { findRawPageNodeById } from './navTreeHelpers';
...
function buildNavigationsForPage(page, allPagesTree, activeNavigations, allNavigationsById, currentPath) {
  ...
  const childPages = rawMatch ? buildNestedPages(rawMatch.node.children || [], currentPath, rawMatch.parentPath) : [];
  ...
  for (const id of Object.keys(allNavigationsById)) {
    navigations.byId[id] = { name: allNavigationsById[id].name, code: allNavigationsById[id].code, data: navData };
  }

...
export async function collectScanSources() {
  ...
  // JSON.stringify escapes quotes (src=\"...\"); unescape so the iframe regex matches.
  const textSources = [
    ...pages.map((p) => ({
      label: `Seite: ${p.slug}`,
      text: `${JSON.stringify(p.blocks)} ${JSON.stringify(p.data)}`.replace(/\\"/g, '"'),
    })),
    ...posts.map((p) => ({
      label: `Blog: ${p.slug}`,
      text: `${p.body || ''} ${JSON.stringify(p.templateData || '')}`.replace(/\\"/g, '"'),
    })),
  ];
